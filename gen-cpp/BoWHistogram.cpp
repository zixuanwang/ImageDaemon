...
void BoWHistogram::load(int64_t rowKey) {
	...
	HbaseAdapter::instance()->loadCell(&strHistogram, GlobalConfig::IMAGE_TABLE,
			strRowKey, GlobalConfig::IMAGE_BOW_HISTOGRAM_COLUMN);
...
void BoWHistogram::save(int64_t rowKey) const {
	...
	HbaseAdapter::instance()->saveCell(strHistogram, GlobalConfig::IMAGE_TABLE,
			strRowKey, GlobalConfig::IMAGE_BOW_HISTOGRAM_COLUMN);
}
