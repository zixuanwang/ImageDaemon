...
typedef struct _ImageDaemon_computeSURFFeature_args__isset {
  _ImageDaemon_computeSURFFeature_args__isset() : rowKey(false) {}
  bool rowKey;
} _ImageDaemon_computeSURFFeature_args__isset;

class ImageDaemon_computeSURFFeature_args {
 public:

  ImageDaemon_computeSURFFeature_args() : rowKey(0) {
  }

  virtual ~ImageDaemon_computeSURFFeature_args() throw() {}

  int64_t rowKey;

  _ImageDaemon_computeSURFFeature_args__isset __isset;

  void __set_rowKey(const int64_t val) {
    rowKey = val;
  }

  bool operator == (const ImageDaemon_computeSURFFeature_args & rhs) const
  {
    if (!(rowKey == rhs.rowKey))
      return false;
    return true;
  ...
  bool operator < (const ImageDaemon_computeSURFFeature_args & ) const;
...
class ImageDaemon_computeSURFFeature_pargs {
 public:


  virtual ~ImageDaemon_computeSURFFeature_pargs() throw() {}
...
class ImageDaemon_computeSURFFeature_result {
 public:
  ...
  ImageDaemon_computeSURFFeature_result() {
  }

  virtual ~ImageDaemon_computeSURFFeature_result() throw() {}


  bool operator == (const ImageDaemon_computeSURFFeature_result & /* rhs */) const
  {
    return true;
  }
  bool operator != (const ImageDaemon_computeSURFFeature_result &rhs) const {
  ...
  bool operator < (const ImageDaemon_computeSURFFeature_result & ) const;
...
class ImageDaemon_computeSURFFeature_presult {
 public:


  virtual ~ImageDaemon_computeSURFFeature_presult() throw() {}
...
class ImageDaemonClient : virtual public ImageDaemonIf {
 public:
  ...
  void computeSURFFeature(const int64_t rowKey);
  void send_computeSURFFeature(const int64_t rowKey);
  void recv_computeSURFFeature();
  void query(std::vector<std::string> & _return, const std::string& imagePath);
...
class ImageDaemonMultiface : virtual public ImageDaemonIf {
 ...
 public:
  ...
  void computeSURFFeature(const int64_t rowKey) {
    size_t sz = ifaces_.size();
    for (size_t i = 0; i < sz; ++i) {
      ifaces_[i]->computeSURFFeature(rowKey);
    }
  }
